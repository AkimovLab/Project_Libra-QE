...
def check_job_status(i,flag1):
    if flag1==0:
        f = open("x%i.scf.out"%i,"r")
        A = f.readlines()
        f.close()
        status = 0
        for s in A:
            ...
            if len(sa)>0 and sa[0]=="JOB" and sa[1]=="DONE.":
                status = 1 # job completed 
    elif flag1==1:
        f = open("x%i.exp.out"%i,"r")
        A = f.readlines()
        f.close()
        status = 0
        for s in A:
...
#
    ...
    print "t0=",t0
    os.system("sbatch submit_templ_x%i.slm"%i) # Run scf calculation first
    ...
    while status !=1: # Until previous job is finished
        ...
        status = check_job_status(i,0) #  status = 1 if the last job is completed, else 0
        print "status",status,"checkjobstatus"
        t0 = t1
    #run_jobs(i,ni)
    print "submitting pw_export job"
    os.system("sbatch submit_templ_x%ie.slm"%i)
    t0 = t1
    status = 0
    while status !=1: # Until previous job is finished
        while (t1 - t0)/T != 1 :  # This means 10 sec interval
            t1=int(time.time())
        #print "Current time = ",int(time.time())," JOB RUN = ",i*ni," to ",i*ni+ni-1
        status = check_job_status(i,1) #  status = 1 if the last job is completed, else 0
        t0 = t1
    print "PW_EXPORT job done"
    # Wait until pw_export job complets
    #run_jobs(i,ni)
    #os.system("sbatch submit_templ_x%ie.slm"%i)
        ...
        while status != 0: #for i in xrange(5):
            ...
            if status == 0:
                ...
                if params["nspin"] == 2:
                    ...
                    occ_bet = fermi_pop(en_bet,nel,params["nspin"],params["electronic_smearing"],0) # Beta on GS
                if params["nspin"] == 1:
