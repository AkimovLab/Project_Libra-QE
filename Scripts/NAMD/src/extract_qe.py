...
def fermi_pop(e,nel,nspin,kT,frmi_indx):
    ##
    ...
    a = MATRIX(N,N)
    if frmi_indx==0: # For alpha in S0 el state and Beta spin
        el_scheme = [0]
    elif frmi_indx==1: # For alpha in S1, or HOMO+1
        el_scheme = [-1,0,1]    
    #el_scheme = [-1,0,1]    
    elif frmi_indx==2:  # For alpha in S2, or HOMO+2
        el_scheme = [-1,1,2]
    #el_scheme = [-1,1,2]
    ...
    for ic in xrange(N):
        ...
        else: # At this point for S1 and S2, we consider finite gap
            pop_av[ic] = pop_tot[0][ic]+pop_tot[2][ic] - pop_tot[1][ic]
...
#
    ...
    # Get gradients
    grads = qe_extract_gradients(A[iforce+2:iforce+2+nat], flag) # For Ethylene and Azobenzene
    #grads = qe_extract_gradients(A[iforce+4:iforce+4+nat], flag)  # For F2PN
